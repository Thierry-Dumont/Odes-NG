...
namespace odes
{
  ...
  ////////////////////////////////////////////////////////////////////
  template<class Fonct> class Radau5omp: 
...
#include "Ivdep.hpp"
    ...
    //! are arrays.
    Radau5omp(bool _sameTestValue,double _atol[],double _rtol[]): 
      ...
      if(ComputeJacobianNumerically)
	{
	  ...
	  cout<<"nbthreads: "<<nbthreads<<endl;
	  RR=new double [nbthreads*n];
	  yy=new double [nbthreads*n];
	}
    }
    //! destructor
    ~Radau5omp()
	...
	throw OdesException("Radau5omp:setFnewt, _fnewt<=uround/tolst",
	...
	{
	  if(nstep>nmax)
	    throw OdesException("Radau5omp() nstep=",nstep,"too large, nmax=",
				   nmax);
